        ...
        while let Some(command) = rx.recv().await {
            match command {
                ...
                SessionCommand::SetSessionModel {
                    ...
                    assert!(
                        switch.skip_prompt_rewrite,
                        "slug normalization must retain the head"
                    );
                    assert_eq!(switch.sampling_config.model, "routing-slug");
...
async fn prepare_web_search_keeps_independent_model_routing() {
    ...
    agent.models_manager.insert_test_entry("search", entry);
...
fn sdk_041_mode_missing_and_closed_session_fails_promptly() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        for id in [sid, acp::SessionId::new("missing")] {
            ...
            .await
            .expect("missing mode request must not hang");
...
fn sdk_041_cancel_target_metadata_and_rewind_compatibility() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        for (meta, expected) in [
            (
                serde_json::json!({"targetPromptId": "p"}),
                CancelHistoryDisposition::KeepIfFront {
                    prompt_id: "p".into(),
                },
            ),
            (
                serde_json::json!({"promptId": "legacy"}),
                CancelHistoryDisposition::Keep,
            ),
            (
                serde_json::json!({"targetPromptId": "p", "rewindIfNoOutput": true}),
                CancelHistoryDisposition::RewindIfNoOutput {
                    prompt_id: Some("p".into()),
                },
            ),
            (
                serde_json::json!({"targetPromptId": "p", "promptId": "p", "rewindIfPristine": true}),
                CancelHistoryDisposition::RewindIfNoOutput {
                    prompt_id: Some("p".into()),
                },
            ),
            ...
            agent
                .cancel(acp::CancelNotification::new(sid.clone()).meta(meta.as_object().cloned()))
                .await
                .unwrap();
        ...
        let err = agent
            .cancel(acp::CancelNotification::new(sid).meta(meta.as_object().cloned()))
            .await
            .unwrap_err();
        assert_eq!(err.code, acp::Error::invalid_params().code);
