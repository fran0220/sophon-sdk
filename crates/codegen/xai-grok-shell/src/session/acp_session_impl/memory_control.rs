...
impl SessionActor {
    ...
    async fn publish_agent_prompt(&self) {
        ...
        // Keep the native prompt context current for future rebuilds, but never
        // replace an explicit SDK prompt with a newly rendered memory section.
        let explicit_prompt = self.explicit_system_prompt.borrow().clone();
        if let Some(prompt) = explicit_prompt {
            super::save_system_prompt(&self.session_info, &prompt);
            self.chat_state_handle.replace_system_head(&prompt).await;
            return;
        }
...
mod tests {
    ...
    async fn memory_prompt_publication_preserves_explicit_sdk_override() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let (actor, _) = super::super::support::build_actor().await;
                actor
                    .handle_replace_system_prompt("SDK authored prompt".to_owned())
                    .await;
                actor.publish_agent_prompt().await;
                let conversation = actor.chat_state_handle.get_conversation().await;
                assert!(matches!(
                    conversation.first(),
                    Some(xai_grok_sampling_types::ConversationItem::System(system))
                        if system.content.as_ref() == "SDK authored prompt"
                ));
            })
            .await;
    }
