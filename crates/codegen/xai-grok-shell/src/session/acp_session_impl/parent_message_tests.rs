...
async fn agent_quiesce_fences_queue_and_steer_before_receipt_or_state_changes() {
    ...
    await_with_timeout(local.run_until(async {
        ...
        actor.tool_context.admission.begin_quiesce();
        ...
        for operation in [
            ...
            actor
                ...
                .await;
            assert_eq!(
                admission_response(response_rx.await),
...
#[tokio::test(flavor = "current_thread")]
async fn peer_fifo_admission_invalidates_cas_and_preserves_idempotent_protected_mutations() {
    use crate::session::commands::{QueueMutation, QueueMutationRequest, QueueMutationResult};
    let local = tokio::task::LocalSet::new();
    await_with_timeout(local.run_until(async {
        let (actor, _gateway_rx) = super::super::support::build_actor().await;
        set_running(&actor, "running").await;
        let stale = actor.tool_context.queue_clock.snapshot();
        ...
        let current = actor.tool_context.queue_clock.snapshot();
        assert_ne!(current, stale);
        let (cmd_tx, cmd_rx) = mpsc::unbounded_channel();
        let (_chat_tx, chat_rx) = mpsc::unbounded_channel();
        // The running task is a stub: this test drives only the command mailbox.
        let (_event_tx, event_rx) = mpsc::unbounded_channel();
        let loop_task =
            crate::util::AbortOnDrop(tokio::task::spawn_local(super::super::run_session(
                actor.clone(),
                cmd_rx,
                chat_rx,
                event_rx,
        ...
        let remove = QueueMutation::Remove {
            id: "parent-message-cas-peer".into(),
            expected_entry_version: 0,
            owner: None,
        };
        for (id, expected, mutation, outcome) in [
            ("stale", stale.clone(), remove.clone(), "conflict"),
            ("protected", current.clone(), remove.clone(), "commit"),
            ("protected", stale, remove, "replay"),
            (
                "protected",
                current,
                QueueMutation::Clear { owner: None },
                "reuse",
            ),
        ] {
            ...
            cmd_tx
                ...
                .expect("actor alive");
            match (outcome, response.await.expect("mutation response")) {
                ("conflict", QueueMutationResult::Conflict { .. }) => {}
                (
                    "commit",
                ...
                ("reuse", QueueMutationResult::OperationIdReused { .. }) => {}
                (_, result) => panic!("unexpected {outcome}: {result:?}"),
            }
        }
        assert!(
            actor
                ...
                .iter()
                .any(
                    |item| item.prompt_id == "parent-message-cas-peer" && item.is_queue_protected()
                )
        );
        drop(loop_task);
    }))
...
async fn receipt_backpressure_waits_before_queue_commit() {
    ...
    await_with_timeout(local.run_until(async {
        ...
        await_with_timeout(async {
            while actor.tool_context.admission.snapshot().active == 0 {
                tokio::task::yield_now().await;
            }
        })
        .await;
        assert_eq!(
            actor.tool_context.admission.snapshot().active,
            1,
            "preparing peer work must be visible to the Agent drain"
        );
        actor.tool_context.admission.begin_quiesce();
...
async fn committed_delivery_queues_protected_fifo_row_with_typed_receipt_identity() {
    ...
    await_with_timeout(local.run_until(async {
        ...
        let (receipt_sink, mut receipt_rx) = mpsc::channel(1);
        let (respond_to, response_rx) = oneshot::channel();
        let before = actor.tool_context.queue_clock.snapshot();
        ...
        let receipt = receipt_rx.recv().await.expect("typed receipt handed off");
        assert_ne!(actor.tool_context.queue_clock.snapshot(), before,
            "peer admission must invalidate pre-admission FIFO CAS versions before acknowledgment");
        assert_eq!(actor.tool_context.admission.snapshot().active, 0);
