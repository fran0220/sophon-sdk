...
mod startup_task_tests {
    ...
    async fn stopping_startup_prevents_late_mcp_prompt_promotion() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let (session, _) = super::super::support::build_actor().await;
                // Hold startup before its state read. A live init claim alone
                // no longer blocks ensure_mcp_tools_initialized in 1.0.24.
                let mut mcp = session.mcp_state.lock().await;
                let _init_claim = mcp.restart_init();
                session.state.lock().await.pending_inputs.push_back(
                    super::super::support::user_item("queued-at-shutdown", "owner"),
                );
                let (completion_tx, _completion_rx) = mpsc::unbounded_channel();
                let tasks = StartupTasks::spawn(&session, completion_tx);
                let promotion = tasks._mcp_init_prompt_promote.0.abort_handle();
                let (started_tx, started_rx) = oneshot::channel();
                let (finished_tx, finished_rx) = oneshot::channel::<()>();
                tasks.mcp_startup.spawn_local(async move {
                    let _ = started_tx.send(());
                    std::future::pending::<()>().await;
                    let _ = finished_tx.send(());
                });
                started_rx.await.expect("MCP followup starts");
                tokio::task::yield_now().await;
                assert!(!promotion.is_finished());
                drop(tasks);
                assert!(matches!(
                    session.startup_tasks.spawn_local(async {
                        panic!("MCP followup must not restart after final exit");
                    }),
                    StartupHandoff::RunLoopEnded
                ));
                assert!(
                    tokio::time::timeout(Duration::from_secs(1), finished_rx)
                        .await
                        .expect("MCP followup aborts")
                        .is_err()
                );
                mcp.complete_init();
                drop(mcp);
                ...
                .await
                .expect("startup promotion aborts");
...
pub(super) async fn run_session(
    ...
    let mut startup_tasks = Some(StartupTasks::spawn(&session, completion_tx.clone()));
                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::GetDrainSnapshot { respond_to } => {
                            ...
                            let _ = respond_to.send(
                                crate::session::commands::SessionDrainSnapshot {
                                    ...
                                    active_work: session.active_work.load(
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            let session_for_mcp = session.clone();
                            if let Some(tasks) = startup_tasks.as_ref() {
                                tasks.mcp_startup.spawn_local(async move {
                                    session_for_mcp
                                ...
                                let _ = respond_to.send(Err(acp::Error::internal_error()
                                    .data("session startup has stopped")));
                            }
                        }
