...
async fn sdk_041_stale_typed_mutations_preserve_edit_hold() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert!(
                !actor
                    .handle_edit_queued_prompt_versioned("p", Some(99), "stale".into(), Some("bob"))
                    .await
            );
            assert_eq!(actor.state.lock().await.edit_holds["p"], held_at);
            assert!(
                !actor
                    .handle_remove_queued_prompt_with_policy("p", 99, Some("alice"), false)
                    .await
            );
            assert_eq!(actor.state.lock().await.edit_holds["p"], held_at);
            let outcome = actor
                ...
                *actor.current_prompt_id.lock().unwrap() = Some("old".into());
                let stale = actor
                    .cancel_running_task(crate::session::CancelOptions {
                        history: crate::session::CancelHistoryDisposition::KeepIfFront {
                            prompt_id: "old".into(),
                        },
