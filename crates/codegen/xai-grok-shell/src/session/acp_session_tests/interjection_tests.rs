...
/// A structured-elicitation answer is not consumed when the UI submits it.
/// It becomes model-visible only at the same safe-point drain used by steer,
/// and its receipt is acknowledged only after that injection completes.
#[tokio::test]
async fn elicitation_answer_is_acknowledged_only_after_safe_point_consumption() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (actor, _gateway_rx) = build_actor().await;
            let (consumed, mut receipt) = tokio::sync::oneshot::channel();
            actor
                .pending_elicitation_answers
                .push(PendingElicitationAnswer {
                    interjection: PendingInterjection {
                        text: "User chose Postgres.".to_string(),
                        attachments: Vec::new(),
                    },
                    claim: ElicitationClaim::default(),
                    consumed,
                });
            ...
            receipt
                .await
                .expect("consumption receipt is sent after injection");

            let conversation = actor.chat_state_handle.get_conversation().await;
            let answer = conversation.last().expect("answer reaches the transcript");
            let ConversationItem::User(answer_item) = answer else {
                panic!("elicitation answer must be a synthetic user message");
            };
            assert_eq!(
                answer_item.synthetic_reason,
                Some(SyntheticReason::Interjection)
            );
            assert!(answer.text_content().contains("User chose Postgres."));
        })
        .await;
}

/// Turn settlement clears answers that missed the final safe point. Dropping
/// their receipt reports truthful `Unanswered` to the coordinator and the text
/// cannot leak into a later Turn.
#[tokio::test]
async fn clearing_unconsumed_elicitation_answer_drops_receipt_without_injection() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (actor, _gateway_rx) = build_actor().await;
            ...
            actor.pending_elicitation_answers.clear();
            assert!(receipt.await.is_err(), "unconsumed receipt must be dropped");
            assert!(!actor.drain_pending_interjections().await);
            assert_eq!(
                actor.chat_state_handle.get_conversation().await.len(),
                before,
                "a cleared answer must not leak into a later conversation step"
            );
        })
        .await;
}

/// Withdrawal and safe-point consumption share one first-wins claim. If the
/// withdrawal wins, the queued answer is discarded and cannot become a user
/// message in the transcript.
#[tokio::test]
async fn withdrawn_elicitation_answer_is_not_injected_at_the_safe_point() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (actor, _gateway_rx) = build_actor().await;
            ...
            actor
                ...
                .push(PendingElicitationAnswer {
                    interjection: PendingInterjection {
                        text: "withdrawn answer".to_string(),
                        attachments: Vec::new(),
                    },
                    claim: claim.clone(),
                    consumed,
            ...
            assert!(claim.claim(), "withdrawal wins the open claim");
            assert!(!claim.claim(), "the answer cannot win the same claim");
            assert!(
                actor.drain_pending_interjections().await,
                "the safe point observed and discarded a queued answer"
            );
            assert!(receipt.await.is_err(), "discarding drops the receipt");
            assert_eq!(
                actor.chat_state_handle.get_conversation().await.len(),
                before,
                "withdrawn answer must not enter the transcript"
            );
        })
        .await;
}
